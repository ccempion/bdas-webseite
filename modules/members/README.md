...
Joining a `netzwerk` group is a request like any other. That group has no
board, so the federal board decides it (ADR 0021 fallback, ADR 0046);
approving it sets the account `active` but does not make it a member
(`isBdasMember`). The app only lets people pick a Hochschulgruppe themselves
(`apps/web/lib/self-service-group.ts`) until the triage wizard brings an entry
point for Förderer.
