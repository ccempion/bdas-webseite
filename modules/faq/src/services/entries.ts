...
const ENTRY_ORDER = [
  asc(faqEntries.section),
  sql`${faqEntries.subgroup} nulls first`,
  ...
  const row = await db.transaction(async (tx) => {
    const [inserted] = await tx
      ...
      .values({
        ...
        position: sql`coalesce((select max(position) from faq_entries where section = ${input.section} and subgroup is not distinct from ${subgroup}), -1) + 1`,
      })
      .returning();
