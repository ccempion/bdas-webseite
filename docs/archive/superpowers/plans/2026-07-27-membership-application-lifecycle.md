...
> **Closed in Phase 2, after the code review.** `isProfileComplete` required
> `member.primaryGroupId != null`, which ADR 0031 takes away from applicants —
> so their profile was never "complete", `/profil` never redirected, and they
> were returned to the wizard they had just finished. The group conjunct is
> gone: completeness now means "the profile row is stamped", and the callers
> that care about the group ask about the group. Task 12 inherits a working
> redirect rather than a bug.
