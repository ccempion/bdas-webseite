...
- **`KalenderBlock.tsx`** _(server component — shape unchanged)._ Fetches
...
- **`EventCalendar.tsx`** _(client island — rewritten)._ Owns two pieces of
...
- **`AgendaList.tsx`** _(client)._ Takes filtered events (already sorted
...
- **`EventAccordion.tsx`** _(client)._ Built on the canonical
...
- **`MonthGrid.tsx`** _(client)._ Monday-first month grid with prev/next month
...
- **`month-grid.ts`** _(pure, testable)._ `buildMonthWeeks(year, month, events)`
...
- **`month-grid.test.ts`** _(new)_ — unit-test `buildMonthWeeks`: Monday-first
