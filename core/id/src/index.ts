...
export function createId(prefix: string, size = 21): string {
  if (!PREFIX_PATTERN.test(prefix)) {
    throw new Error(`Invalid id prefix '${prefix}': must be 2-5 lowercase ASCII letters`);
  }
  return `${prefix}_${nanoid(size)}`;
}

/** Type guard: checks whether an id has the expected prefix. */
export function hasPrefix<P extends string>(id: string, prefix: P): id is `${P}_${string}` {
